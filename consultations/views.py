...
def consult_list(request):
    u=request.user; qs=Consultation.objects.select_related('patient','doctor')
...
def consult_create(request):
    ...
    if form.is_valid():
        c=form.save(commit=False); c.doctor=request.user; c.save()
...
def consult_detail(request, pk):
    ...
    return render(request,'consultations/detail.html',{'c':c,
        'prescriptions':c.prescriptions.all(), 'lab_orders':c.lab_orders.all()})
...
def vitals_create(request, patient_id):
    ...
    if form.is_valid():
        v=form.save(commit=False); v.patient=p; v.recorded_by=request.user; v.save()
        return redirect('patients:detail', pk=patient_id)
